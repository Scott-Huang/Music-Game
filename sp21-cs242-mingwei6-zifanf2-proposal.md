...
| Game Layout View | 5 | -1pt for the layout is not resizable <br /> -1pt for each missing components (should contain circles, their falling track, keys, scores, background, and music, up to 3 pt) <br /> -1 pt for game interface cannot be displayed normally |
| Circle Rendering | 2 | -2pt for circles flowing not rendered <br /> -1pt for circles not moving correctly |
| Game Effects | 2 | -2pt for no read from keyboard <br /> -1pt for no effect on key pressed <br /> (The key in screen should light up when its correspoding key press occurrs.) |
| Accuracy Detection | 1 | -1pt for no accuracy detection |
| Game Model Design | 3 | -2pt for no clear model design <br /> -1pt for there is no clear seperation between the view and model |
...
| Key Press Rendering | 4 | -3pt for no effects shown <br /> -2pt for no pomp-out information about the accuracy <br /> -2pt for circles still remain after pressing keys |
| Game Play | 4 | -4pt for game not integrated at all <br />  -3pt for game cannot run at all <br /> -1pt for game cannot start correctly <br /> -1pt for game cannot end correctly <br /> -1pt for not counting scores <br /> -1pt for any other small errors (up to 2pt) |
| Result Display | 2 | -2pt for no result desplaying <br /> -1pt for result not shown proporly |
| Game Setting | 3 | -2pt for no setting view before game starts <br /> -1pt for settings cannot be proporly set for games |
| Error Handling | 1 | +1pt for handling invalid music file |
| Unit Test | 2 | +0.5pt for every unit test |
| Mannual Test | 8 | +0.5pt for each manual test (up to 6pt) <br /> +2pt for test an entire game with a song playing |

###### Week 3
| Category | Total Scores Allocated |           Detailed Rubrics            |
| -------- | ---------------------- | ------------------------------------- |
| Beat Detection | 4 | -4pt for no beat detection <br /> -2pt for not using detected beats |
| Circles Falling Patterns | 6 | +0.1pt for each pattern (up to 3) <br /> +2pt for auto-generate patterns <br /> +1pt for the format of patterns can be used for generating games |
| Generating Game | 5 | -5pt for game not generated at all <br /> -3pt for game randomly generated without considering rhythm |
| Unit Test | 3 | +0.5pt for each unit test |
| Mannual Test | 7 | +1pt for each autogenerated games tested |

Patterns explanation:
The rhythm only specifies the time that users should press keys, but which key should be pressed is left to be decided by the game. 
A pattern is a set of consecutive key pressing. They should not be too random in order to let players better perceive the rhythm.
Different patterns should have different sequences of key pressing.

###### Week 4
(I feel that this will likely to be changed.)
| Category | Total Scores Allocated |           Detailed Rubrics            |
| -------- | ---------------------- | ------------------------------------- |
| Additional Functions <br /> (Can be changed) | 5 | +2pt for each customized settings (keys, speed, input backgrounds) <br /> +3pt for dynamic background (incompatible with input backgrounds) |
| Background | 4 | -4pt for no background <br /> -2pt for no background selection |
| Actual Game | 6 | -6pt for game not playable at all <br /> -4pt for game is not behaving normally <br /> -3pt for game does not have rhythm <br /> -1pt for small bugs |
